...
import com.mobile.store.service.implementation.CustomerService;
...
public class CustomerController {
	...
	@GetMapping("/mobilecover")
	public String viewMobileCover(HttpSession session,ModelMap map) {
		return customerService.viewMobileCover(session,map);
	}
...
//	}
	...
	public String resetpassword() {
		return "reset-password-request.html";
	}
	
	@PostMapping("/request-reset-password")
	public String requestResetPassword(@RequestParam String email, HttpSession session) {
	    return customerService.requestPasswordReset(email, session);
	}
	
	@PostMapping("/verify-reset-password")
	public String verifyAndResetPassword(@RequestParam int otp,@RequestParam String newPassword,@RequestParam String confirmPassword,HttpSession session) {
	    return customerService.verifyOtpAndResetPassword(otp, newPassword, confirmPassword, session);
	}
	
	@GetMapping("/mobilebrands")
	public String viewMobileBrands(HttpSession session,ModelMap map) {
		return customerService.viewMobileBrands(session,map);
	}
	
	@GetMapping("/mobile-cover/mobilebrand")
	public String viewMobileCoversByBrand(@RequestParam("mobilebrand") String mobilebrand, HttpSession session,  ModelMap map) {
		return customerService.viewMobileCoversByBrand(mobilebrand,session,map);
	}
